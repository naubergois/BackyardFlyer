...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        if self.flight_state == States.TAKEOFF:
            if -1.0 * self.local_position[2] > 0.95 * self.target_position[2]:
                ...
                self.all_waypoints=self.random_behaviour()
    ...
    def velocity_callback(self):
        if self.flight_state == States.LANDING:
            if self.global_position[2] - self.global_home[2] < 0.05:
                if abs(self.local_position[2]) < 0.005:
    ...
    def random_behaviour(self):
        ...
        for i in range(100):
            ...
            z_random=random.randint(30,50)

            print(x_random)
            print(y_random)
            print(z_random)

            local_waypoints.append([x_random,y_random,z_random])

        return local_waypoints

    def calculate_box(self):
        print("Setting Home")
        local_waypoints = [[10.0, 10.0, 6.0], [10.0, 20.0, 3.0], [0.0, 10.0, 6], [0.0, 0.0, 6.0]]
        return local_waypoints
